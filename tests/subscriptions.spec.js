...
test('should stop subscriptions', async () => {
    ...
    await delay(1000);
    expect(fetch).toBeCalledTimes(1);
    const deleteRes = await sofa.fetch(`http://localhost:4000/api/webhook/${resBody.id}`, {
        method: 'DELETE',
    });
    ...
    const sofa = useSofa({
        ...
        schema: createSchema({
            ...
            resolvers: {
                Subscription: {
                    onBookBy: {
                        subscribe: (root, args, context, info) => pipe(pubsub.subscribe(BOOK_ADDED), filter((payload) => payload.onBookBy.author === args.author))
                    },
                },
            },
        }),
