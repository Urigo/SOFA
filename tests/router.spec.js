...
test('should work with Query and variables', async () => {
    ...
    expect(spy.mock.calls[0][1]).toEqual({ id: 'test-id' });
});
test('should work with input objects in URL params via GET', async () => {
    ...
    const res = await sofa.fetch('http://localhost:4000/api/users?where={"nested":{"id":"test-id"}}');
    expect(res.status).toBe(200);
    const resBody = await res.json();
    expect(resBody).toEqual([testUser]);
...
test('should work with Mutation + Query', async () => {
    ...
    const res = await sofa.fetch('http://localhost:4000/api/add-random-food?name=test', {
        method: 'POST',
    });
    const resBody = await res.json();
    expect(resBody).toEqual(pizza);
    expect(spy.mock.calls[0][1]).toEqual({ name: 'test' });
});
test('should overwrite a default http method on demand', async () => {
    ...
    const mutationRes = await sofa.fetch('http://localhost:4000/api/add-random-user', {
        method: 'GET',
    });
    ...
    const sofa = useSofa({
      ...
      `,
        ...
        errorHandler(errs) {
            ...
            let status = 500;
            if (errs[0] instanceof GraphQLError &&
